...
export default async function CheckoutPage({ params, searchParams }: CheckoutPageProps) {
    ...
    const hotelImages = Array.isArray(hotel.images)
        ...
        : [];
    const hotelPrimaryImage = hotelImages[0] ?? null;
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                            ...
                            <p className="mt-2 text-zinc-500 dark:text-zinc-400">
                                Review your stay details, enter guest information, then either pay now or send a pre-book request for offline follow-up.
                            </p>
                        </div>
                    ...
                    <div className="mt-4 space-y-2">
                        {selectedRooms.map((room) => (
                            <div key={room.id} className="flex items-center justify-between rounded-2xl bg-zinc-50 px-3 py-3 text-sm dark:bg-zinc-800/60">
                                ...
                                <span className="shrink-0 font-black text-zinc-500">{formatUsd(room.price * room.quantity * nights)}</span>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="space-y-8 lg:col-span-2">
                        ...
                        <div className="flex items-start gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 dark:border-blue-800/50 dark:bg-blue-900/20">
                            ...
                            <p className="text-sm text-blue-700 dark:text-blue-300">
                                <strong>Booking Options:</strong> Pay now for instant confirmation, or choose pre-book to submit a request and let our team arrange payment with you directly.
                            </p>
                        </div>
                    </div>
                                ...
                                <div className="space-y-6 p-6">
                                    ...
                                    <div className="space-y-3 border-t border-zinc-100 pt-4 dark:border-zinc-800">
                                        ...
                                        {selectedRooms.map((room) => (
                                            <div key={room.id} className="flex items-center justify-between text-sm">
                                                ...
                                                <span className="text-zinc-500">{formatUsd(room.price * room.quantity * nights)}</span>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="space-y-2 pt-2">
                                        ...
                                        <div className="mt-4 flex items-end justify-between border-t border-zinc-100 pt-4 dark:border-zinc-800">
                                            ...
                                            <span className="text-2xl font-black text-blue-600">{formatUsd(totalPrice)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
